...
public class SimpleTabList extends ComTitleTabList implements PersonalizadaTabList {
    public static int MAXIMUM_ITEMS = 4 * 20;
    ...
    public SimpleTabList(Tab tabbed, Player player, int maxItems, int minColumnWidth, int maxColumnWidth) {
        super(player);
        Preconditions.checkArgument(maxItems <= MAXIMUM_ITEMS, "maxItems n�o pode passar m�ximo do cliente de " + MAXIMUM_ITEMS);
        Preconditions.checkArgument(minColumnWidth <= maxColumnWidth || maxColumnWidth < 0, "minColumnWidth n�o pode ser maior do que maxColumnWidth");
    ...
    private void validateIndex(int index) {
        Preconditions.checkArgument(index > 0 || index < getMaxItems(), "�ndice n�o em alcance permitido");
    }
